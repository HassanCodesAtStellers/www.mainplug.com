  ...
  var MDTimePicker = function (el, config) {
    ...
    if (_.config.clearBtn) {
      picker.clockHolder.buttonsHolder.btnClear.click(function () {
        _.input.val("").attr("data-time", null).attr("value", "");
  ...
  MDTimePicker.prototype = {
    ...
    setup: function () {
      ...
      for (var i = 0; i < _hours; i++) {
        var value = i + 1,
          deg =
            ((HOUR_START_DEG + i * HOUR_DEG_INCR) % END_DEG) -
            (_.config.is24hour && value < 13 ? 15 : 0),
          is24 = value === 24,
          hour = $(
        ...
        hour.find("span").click(function () {
          var _hour = parseInt($(this).parent().data("hour")),
            ...
            _value = _.config.is24hour
              ? _hour
              : (_hour +
                  ((_selectedT === "PM" && _hour < 12) ||
                  (_selectedT === "AM" && _hour === 12)
                    ? 12
                    : 0)) %
                24,
      ...
      wrapper.attr(
        ...
        _.input.data("theme") ||
          _.config.theme ||
          $.fn.mdtimepicker.defaults.theme
      );
    ...
    setHour: function (hour) {
      ...
      this.timepicker.timeHolder.hour.text(
        is12Hour ? _selectedH : this.selected.format("hh")
      );
    ...
    setPeriod: function (period) {
      ...
      this.timepicker.clockHolder.am[
        _period === "AM" ? "addClass" : "removeClass"
      ]("active");
      this.timepicker.clockHolder.pm[
        _period === "PM" ? "addClass" : "removeClass"
      ]("active");
    },
    ...
    isDisabled: function (hour, minute, renderMode) {
      ...
      if (minT) {
        min = new Date(
          ...
          now.getDate(),
          minT.getHour(),
          hourView && renderMode ? 0 : minT.getMinutes(),
          0,
          0
        );
      }
      ...
      if (maxT) {
        max = new Date(
          ...
          now.getDate(),
          maxT.getHour(),
          hourView && renderMode ? 0 : maxT.getMinutes(),
          0,
          0
        );
      }
    ...
    show: function () {
      ...
      setTimeout(function () {
        ...
        that.timepicker.wrapper.removeClass("animate").focus();
    ...
    triggerChange: function (data) {
      this.input
        .trigger($.Event("timechanged", data))
  ...
  $.fn.mdtimepicker = function () {
    ...
    return $(this).each(function (idx, el) {
      ...
      if (typeof arg0 === "string")
        picker[arg0].apply(
          picker,
          Array.prototype.slice.call(mdtp_args).slice(1)
        );
    });
  };
